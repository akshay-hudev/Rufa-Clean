...
import { vultureAnalyzer } from "../src/indexing/analyzers/vulture";
...
async function main(): Promise<void> {
  ...
  if (!repoSlug) {
    throw new Error(
      "Repository slug argument is required. Usage: npx ts-node scripts/test-vulture.ts <repo-slug>",
    );
  }
  ...
  try {
    if (!vultureAnalyzer.canAnalyze(clone.localPath)) {
      throw new Error(
        `Repository "${repoSlug}" does not have Python files or a manifest in a supported project root`,
      );
    }

    const findings = await vultureAnalyzer.analyze(clone.localPath);
    console.log("Vulture findings:", findings);
  } finally {
    await clone.cleanup();
